...
function SocialLinks({footer=false}){return <div className={footer?'socialLinks footerSocialLinks':'socialLinks'}>{socialLinks.map(s=><a className={'socialIcon '+s.cls} key={s.name} href={s.href} target="_blank" rel="noopener noreferrer" aria-label={s.name} title={s.name}><span>{s.name==='Instagram'?'◎':s.name==='TikTok'?'♪':s.name==='Facebook'?'f':'X'}</span></a>)}</div>}
