...
export default function Home(){
  ...
  const visibleLive=liveExpanded?shownLive:shownLive.slice(0,5);
  const visibleUpcoming=fixturesExpanded?shownUpcoming:shownUpcoming.slice(0,4);
  ...
  return <main>
    ...
    <section id="scores" className="section"><div className="sectionTitle"><div><span className="liveDot"></span> LIVE SCORES</div><small>{liveLoading?'LOADING LIVE SCORES…':liveError?'LIVE FEED RETRYING':scoresConfigured?'TOP LEAGUES FIRST · AUTO-REFRESHING':'API NOT CONNECTED'}</small></div><FootballBrowser allLeagues={allLeagues} country={scoreCountry} setCountry={setScoreCountry} leagueId={scoreLeague} setLeagueId={setScoreLeague} label="Live scores by country"/>{liveLoading?<div className="empty">Checking live matches…</div>:liveError?<div className="empty">Live scores are temporarily unavailable. We’ll retry automatically.</div>:<><MatchGroups matches={visibleLive} onOpen={setMatchId} emptyText={scoresConfigured?'No matches are live in this selection right now.':'The football data connection is not configured.'}/>{shownLive.length>5&&<button className="primary" style={{display:'block',margin:'22px auto 0',minWidth:220}} onClick={()=>setLiveExpanded(v=>!v)}>{liveExpanded?'SHOW LESS LIVE SCORES':`SHOW MORE LIVE SCORES (${shownLive.length-5})`}</button>}</>}</section>

    <section id="fixtures" className="section"><div className="sectionTitle"><div>TODAY & UPCOMING FIXTURES</div><small>TOP LEAGUES FIRST · YOUR LOCAL TIME</small></div><FootballBrowser allLeagues={allLeagues} country={fixtureCountry} setCountry={setFixtureCountry} leagueId={fixtureLeague} setLeagueId={setFixtureLeague} label="Fixtures by country"/><MatchGroups matches={visibleUpcoming} onOpen={setMatchId} emptyText="No upcoming fixtures in this selection for the current two-day window."/>{(shownUpcoming.length>4||shownRecent.length>0)&&<button className="primary" style={{display:'block',margin:'22px auto 0',minWidth:220}} onClick={()=>setFixturesExpanded(v=>!v)}>{fixturesExpanded?'SHOW LESS FIXTURES':`SHOW MORE FIXTURES (${Math.max(0,shownUpcoming.length-4)+shownRecent.length})`}</button>}{fixturesExpanded&&shownRecent.length>0&&<><div className="subTitle">LATEST RESULTS</div><MatchGroups matches={shownRecent} onOpen={setMatchId} emptyText=""/></>}</section>
