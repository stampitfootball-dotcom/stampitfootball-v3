...
function SocialMark({name}){if(name==='Instagram')return <svg viewBox="0 0 24 24" aria-hidden="true"><rect x="4" y="4" width="16" height="16" rx="5" fill="none" stroke="currentColor" strokeWidth="2"/><circle cx="12" cy="12" r="3.5" fill="none" stroke="currentColor" strokeWidth="2"/><circle cx="17.3" cy="6.8" r="1.1" fill="currentColor"/></svg>;if(name==='TikTok')return <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M14 4v10.2a4.2 4.2 0 1 1-3.2-4.08v2.25a2.1 2.1 0 1 0 1.1 1.83V4h2.1c.25 2.2 1.55 3.6 3.9 4.05v2.1c-1.55-.18-2.85-.8-3.9-1.85V4z" fill="currentColor"/></svg>;if(name==='Facebook')return <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M13.8 21v-8h2.7l.4-3h-3.1V8.1c0-.87.24-1.46 1.55-1.46H17V4a22 22 0 0 0-2.4-.13c-2.38 0-4 1.45-4 4.12V10H8v3h2.6v8h3.2z" fill="currentColor"/></svg>;return <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M5 4l14 16M19 4L5 20" fill="none" stroke="currentColor" strokeWidth="2.4" strokeLinecap="round"/></svg>}
function SocialLinks({footer=false}){return <div className={footer?'socialLinks footerSocialLinks':'socialLinks'}>{socialLinks.map(s=><a className={'socialIcon '+s.cls} key={s.name} href={s.href} target="_blank" rel="noopener noreferrer" aria-label={s.name} title={s.name}><SocialMark name={s.name}/></a>)}</div>}
...
function AuthModal({close,reason='Join Stamp It Football',session,onSession}){
  ...
  const submit=async e=>{e.preventDefault();setBusy(true);setError('');setMessage('');try{
    ...
    else if(mode==='join'){const d=await sbAuth('signup?redirect_to='+encodeURIComponent('https://stampitfootball.com/'),{email,password,data:{display_name:name}});if(d.access_token){const s={access_token:d.access_token,refresh_token:d.refresh_token,user:d.user};localStorage.setItem('stampit_session',JSON.stringify(s));onSession(s);close()}else setMessage('Account created. Check your email to confirm your account.');}
  ...
  return <main className="dashboardSite">
    ...
    <section id="football" className="footballDashboard v2FootballHub">
      ...
      <div className="dashboardContent">
        ...
        <div className="footballHero"><strong><span>THE HOME OF</span><em>FOOTBALL</em></strong><span>NEWS&nbsp;&nbsp;•&nbsp;&nbsp; SCORES&nbsp;&nbsp;•&nbsp;&nbsp; FIXTURES&nbsp;&nbsp;•&nbsp;&nbsp; PREDICTIONS</span></div>
